...
// added from jrm fashionhouse gig on 20/08/2025 around 173 days after these

function tabSwitchB(no,series,dis_hidden,dis_visible,btns,class_activebtn,class_toreplace) {
	...
	for (var i = 0; i < items.length; i++) {
		items[i].style.display = dis_hidden;
	}

	items[no].style.display = dis_visible;
	...
	var thebtns = document.querySelectorAll(btns);

	class_activebtn = class_activebtn == undefined ? "active" : class_activebtn;
	class_toreplace = class_toreplace == undefined ? "inactive" : class_toreplace;

	thebtns.forEach(el => {
		if(el.className != ""){
			el.classList.remove(class_activebtn);
			el.classList.add(class_toreplace);
		}
	})

	if(thebtns[no] != undefined){
		thebtns[no].classList.add(class_activebtn);
		thebtns[no].classList.remove(class_toreplace);
	}
}
